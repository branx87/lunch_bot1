...
Buttons use ACTION=SEND — the only mode that works reliably in
both Bitrix24 web and mobile clients.  The button label becomes
the chat message text, so labels are kept in Russian (user-visible).
"""
import asyncio
import base64
import logging
...
def _btn(text: str, bg: str = "#29619b") -> dict:
    """ACTION=SEND button — sends the label as a chat message when clicked."""
    return {"TEXT": text, "ACTION": "SEND", "BG_COLOR": bg, "TEXT_COLOR": "#fff"}


KB_MAIN_ADMIN = _kb(
    [_btn("📊 Отчёты"), _btn("📋 Заказы сегодня")],
    [_btn("🛒 Заказать", "#2a7a2a"), _btn("📋 Мои заказы")],
    [_btn("🍽 Меню")],
)
...
KB_PERIOD = _kb(
    [_btn("📅 За сегодня", "#3b7abf"), _btn("📆 За месяц", "#3b7abf")],
    [_btn("🏠 Главное меню", "#555")],
)

KB_RTYPE_ADMIN = _kb(
    [_btn("👨‍💼 Админский", "#29619b"),
     _btn("💰 Бухгалтерский", "#5c7a3e"),
     _btn("📦 Поставщика", "#7a5c3e")],
    [_btn("🏠 Главное меню", "#555")],
)

KB_RTYPE_LIMITED = _kb(
    [_btn("📊 Мой отчёт", "#29619b")],
    [_btn("🏠 Главное меню", "#555")],
)

KB_MONTH_RANGE = _kb(
    [_btn("📅 Текущий месяц", "#3b7abf"), _btn("📆 Прошлый месяц", "#3b7abf")],
    [_btn("🏠 Главное меню", "#555")],
)
...
def _build_select_day_kb() -> list:
    ...
    for day in week:
        ...
        buttons.append(_btn(f"📅 {label}", "#3b7abf"))
    rows = [buttons[i:i+2] for i in range(0, len(buttons), 2)]
    rows.append([_btn("🏠 Главное меню", "#555")])
    return _kb(*rows)
...
def _order_view_kb(has_order: bool, can_modify: bool) -> list:
    ...
    if not has_order:
        if can_modify:
            rows.append([_btn("✅ Заказать 1 порцию", "#2a7a2a")])
    else:
        if can_modify:
            rows.append([
                _btn("➕ Добавить порцию", "#2a7a2a"),
                _btn("➖ Убрать порцию",   "#7a5c3e"),
            ])
            rows.append([_btn("❌ Отменить заказ", "#7a2a2a")])
    rows.append([_btn("🏠 Главное меню", "#555")])
    return _kb(*rows)
...
async def handle_message(
    ...
    _employee_ctx_cmds  = {"✅ заказать 1 порцию", "➕ добавить порцию",
                           "➖ убрать порцию", "❌ отменить заказ"}
    ...
    _is_employee_action = (
        ...
        (step in _employee_steps and (
            ...
            bool(re.match(r'^📅 ', raw)) or
            bool(re.match(r'^❌ отменить \d+\.\d+', raw))
        ))
    )
...
async def _handle_employee_inner(
    ...
    # Day selection: button label starts with "📅 "
    _day_btn = re.match(r'^📅 (.+)$', raw)
    if _day_btn or step in (STEP_SELECT_DAY, STEP_ORDER_VIEW):
        if _day_btn:
            # Find offset by matching label in week menus
            from services.menu_service import get_week_menus as _gwm
            DAYS_SHORT = ["Пн", "Вт", "Ср", "Чт", "Пт", "Сб", "Вс"]
            week = _gwm(CONFIG)
            day_offset = None
            for day in week:
                ...
                day_short = DAYS_SHORT[day["target_date"].weekday()]
                label = f"{'сегодня' if offset == 0 else 'завтра' if offset == 1 else day_short} {date_str}"
                if _day_btn.group(1).lower() == label:
                    day_offset = offset
                    break
    ...
    # Cancel by date from button label: "❌ отменить DD.MM (N порц.)"
    _cancel_btn = re.match(r'^❌ отменить (\d{2}\.\d{2}) \((\d+) порц\.\)$', raw)
    if step == STEP_MY_ORDERS and _cancel_btn:
        date_part = _cancel_btn.group(1)
